# build-signed.ps1
# Script to build and sign the EnvironmentSetup Electron app with a Sectigo USB Token
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        # Verify if the output file exists (portable exe)
        $exeFiles = Get-ChildItem "dist" -Filter "*portable*.exe"
        if ($exeFiles) {
